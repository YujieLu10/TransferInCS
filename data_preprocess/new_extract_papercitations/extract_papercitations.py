...
cited_result_CHI, cited_result_UBI, cited_result_CSCW, cited_result_IMCL, cited_result_UIST = [], [], [], [], []

files = os.listdir("/data4/pcs/papercitations")
for file in files:
    print(file)
    cnt = 0
    #paper_citation_file = pd.read_csv(os.path.join("/data1/mag_data/splited_papercitations",file), nrows=10) 
    paper_citation_file = open(os.path.join("/data4/pcs/papercitations", file))
    ...
    for row in iter_f:
        ...
        # if paperid in data_UBI:# or paperid2 in data_UBI:
        #     result_UBI.append(list(paperrow_str))
        # if paperid in data_CSCW:# or paperid2 in data_CSCW:
        #     result_CSCW.append(list(paperrow_str))
        # if paperid in data_IMCL:# or paperid2 in data_IMCL:
        #     result_IMCL.append(list(paperrow_str))
        # if paperid in data_UIST:# or paperid2 in data_UIST:
        #     result_UIST.append(list(paperrow_str))

        if paperid2 in data_CHI:
            cited_result_CHI.append(list(paperrow_str))
        if paperid2 in data_UBI:
            cited_result_UBI.append(list(paperrow_str))
        if paperid2 in data_CSCW:
            cited_result_CSCW.append(list(paperrow_str))
        if paperid2 in data_IMCL:
            cited_result_IMCL.append(list(paperrow_str))
        if paperid2 in data_UIST:
            cited_result_UIST.append(list(paperrow_str))
...
result_pd = pd.DataFrame(data=cited_result_CHI, columns=['citingpaperid', 'citedpaperid'])
result_pd.to_csv('papercited_CHI.tsv')
result_pd = pd.DataFrame(data=cited_result_UBI, columns=['citingpaperid', 'citedpaperid'])
result_pd.to_csv('papercited_UBI.tsv')
result_pd = pd.DataFrame(data=cited_result_CSCW, columns=['citingpaperid', 'citedpaperid'])
result_pd.to_csv('papercited_CSCW.tsv')
result_pd = pd.DataFrame(data=cited_result_IMCL, columns=['citingpaperid', 'citedpaperid'])
result_pd.to_csv('papercited_IMCL.tsv')
result_pd = pd.DataFrame(data=cited_result_UIST, columns=['citingpaperid', 'citedpaperid'])
result_pd.to_csv('papercited_UIST.tsv')
